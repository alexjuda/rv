...
from ..models.github import PRComment, Review, Thread, ThreadComment
...
class CompleteIDs:
    ...
    def exec(self, id_prefix: str) -> list[tuple[str, str]]:
        ...
        pr_comms = self._store.get_pr_comments_matching(id_prefix)
        reviews = self._store.get_reviews_matching(id_prefix)

        return (
            [self._ui.format_thread(th) for th in threads]
            + [self._ui.format_thread_comment(comm) for comm in th_comms]
            + [self._ui.format_pr_comment(comm) for comm in pr_comms]
            + [self._ui.format_review(rv) for rv in reviews]
        )
